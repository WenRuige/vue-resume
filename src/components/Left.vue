...
.article_description { text-indent:2em;}
      ...
      test: function () {
        ...
        this.$data.id = setInterval(this.loop, 500)
      }, loop: function () {
        ...
        this.$data.message += this.$data.splitArray[this.$data.counter] != "" ? "<span>" + this.$data.splitArray[this.$data.counter].trim() + "</span>" : '';
        ...
        dataObj["right_content"] = "margin/20px";
        //声明数组,可实现
        this.$set(this.$data.right_content, "margin", "20px")
        //定时更新style数据
        setInterval(this.appendStyle, 1000)
        //this.randColor()
        //随机append数组内的颜色
      }, appendStyle: function () {

        this.$set(this.$data.styleObject, "color", color[randomNum])
      }
      , randColor: function () {
